---
title: How to Read This Research
aliases:
  - Reader Guide
  - Orientation Guide
  - Project Guide
description: A short orientation guide for understanding the Copley family research vault, its evidence labels, major pages, and open questions.
date: 2026-04-28
tags:
  - "#copley-family"
  - "#genealogy"
  - "#index"
---

# How to Read This Research

This site is a living genealogical research project, not a finished family history. It combines family narrative, person profiles, source tracking, open research questions, and working hypotheses. Some claims are well documented; others are active leads that are useful but not yet proven.

Use this page as the map before diving into the details.

## The Main Layers

| Page | What It Answers |
|---|---|
| [[The Copley Family Narrative]] | What is the family story in readable prose? |
| [[Who Was Alive When]] | Who overlapped with whom, and which generations were alive at the same time? |
| [[Family Tree]] | How do the people and generations connect? |
| [[Visual Story Atlas]] | What are the main relationships, migrations, evidence clusters, and research quests at a glance? |
| [[People Directory]] | Where are the individual profile pages? |
| [[Sources and Evidence Index]] | What do we think we know, how strong is the evidence, and where is the proof trail? |
| [[Research Priorities and Action Items]] | What research would most improve the project next? |
| [[Topics and Themes]] | What historical themes explain the family story? |
| [[Search Index]] | Where can I look up names, places, topics, and keywords? |

## Start Here by Reader Type

| If You Are... | Start With |
|---|---|
| A family member looking for the big picture | [[The Copley Family Narrative]], then [[Family Tree]] |
| Trying to understand chronology or overlapping lives | [[Who Was Alive When]], then [[Family Tree]] |
| Looking for a specific person | [[People Directory]], then [[Search Index]] |
| Checking whether a claim is proven | [[Sources and Evidence Index]] |
| Interested in Irish origins | [[Topics/Captain John Copley Research]], [[Places/Kilgefin Ireland]], and [[Places/Kinawley Ireland]] |
| Interested in the Lewis County immigrant community | [[Topics/Murray Settlement]] and [[Topics/Murray Settlement Research Roadmap]] |
| Looking for what to research next | [[Research Priorities and Action Items]] and [[Tier 1 Research Execution Kit]] |
| Reviewing the project as a researcher or AI agent | [[CLAUDE]] first, then [[Sources and Evidence Index]] |

## Evidence Labels

The project uses confidence labels so readers can separate documented fact from useful but unfinished research.

| Label | How To Read It |
|---|---|
| **Verified** | Supported by a named primary source, source image, record extract, or clearly identified contemporary source. |
| **Strong working conclusion** | Multiple independent clues point the same way, but the direct identity record is still missing. |
| **Plausible** | A reasonable synthesis or family-memory interpretation, but not yet proven. |
| **Oral / family tradition** | Preserved in family memory or family-authored material; useful, but should be corroborated before treated as fact. |
| **Unresolved** | Important question remains open. |
| **Contradicted / rejected** | Earlier claim is contradicted by later evidence and should not be repeated as fact. |

When in doubt, check [[Sources and Evidence Index]] before relying on a claim.

## The Core Story

The central line begins with [[People/Michael Copley Sr|Michael Copley Sr.]], born around 1813 and associated by family tradition with [[Places/Kilgefin Ireland|Kilgefin, County Roscommon]], and [[People/Ann Copley|Ann Copley]], remembered as Ann Munday but now treated as likely Murray pending direct proof. Their family settled in [[Places/Lewis County West Virginia|Lewis County, West Virginia]], where land, Catholic community formation, Irish migration networks, and later oil development shaped the family's American story.

The most developed 19th- and 20th-century line runs through [[People/John Copley|John Copley]] and [[People/Mary Ellen Dolan Copley|Mary Ellen Dolan Copley]], then into the Sardo, Ruland, Stephen Copley, and Tom Copley branches.

## The Biggest Open Questions

These questions explain why some pages use cautious wording:

| Question | Current Status | Best Control Page |
|---|---|---|
| Who were Michael Copley Sr.'s Irish parents? | Strong origin lead, parentage unresolved | [[Topics/Captain John Copley Research]] |
| Was Ann recorded as Munday actually from a Murray family? | Strong working conclusion, direct record missing | [[People/Ann Copley]] |
| Which Kinawley Murray household did Ann belong to? | Open | [[Research Priorities and Action Items]] |
| Did a Murray-led settlement network anchor the Lewis County Irish Catholic community? | Plausible high-value framework | [[Topics/Murray Settlement]] |
| How do the Fairymount Copelys connect to Michael Sr.? | Verified local Copely presence, relationship unresolved | [[Places/Kilgefin Ireland]] |
| Was Mary Copely Giblin close kin to Michael Sr.? | Verified person, plausible relationship | [[People/Mary Copely Giblin]] |
| Did Copley No. 1 oil income support later education and mobility? | Oil strike verified, financial effect plausible | [[Topics/1900 Copley Oil Strike]] |
| Does the line descend from Captain John Copley? | Plausible long-range hypothesis, not proven generation by generation | [[Topics/Bredon Descent]] |

## Reading Rule

Read the narrative for the story, but use the evidence index to decide how much weight to give a claim. A page can be valuable even when it is not final: in genealogy, a clearly labeled unresolved lead is often the thing that makes the next discovery possible.
