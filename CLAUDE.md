...
- The Partlow / Rude identity path has changed: Frank Partlow = Nollie Franklin Partlow, Alice Rude = Mary Alice Rude Partlow, and Marion Elizabeth Partlow's parentage are now proved for project purposes by the web-only chain in `References/Partlow Rude Online Identity Chain.md`. The original 1880 census image is also found at `References/1880 Marion McDonald Partlow Household.md`, confirming Nollie Frank as son of Marion Partlow. The reported 10 Jun 1900 Clark County marriage return remains unlocated online, but it is now an optional ceremony-detail target rather than the identity-control target. Do not reassign generic FamilySearch, generic 1880 / 1900 census, broad Ancestry, broad Marshall-newspaper, or Clerk / Marshall Library tasks as the next web task for identity. For web work, move upstream: capture original 1850 / 1860 census images, search *The Partlow family and connections*, search DAR GRS / pension / probate / land paths for Benjamin and Jacob, and verify the Spotsylvania Will Book A p. 975 abstract.
