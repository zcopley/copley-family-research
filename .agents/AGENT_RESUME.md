...
- The Frank -> Marion McDonald Partlow link is online-supported by derivative census citations, but the original 1880 census image should still be captured.
- The critical Partlow proof gaps are now John H. -> Jacob and Jacob -> Benjamin. The Clark County marriage return is optional ceremony-detail work, not the identity-control target.
...
- Partlow next steps:
  - Preserve the June 2026 result that Frank Partlow = Nollie Franklin Partlow, Alice Rude = Mary Alice Rude Partlow, and Marion Elizabeth's parentage are now proved for project purposes by web records. Do not repeat generic FamilySearch, generic 1900 census, broad Ancestry, broad Marshall-newspaper, or Clerk / Marshall Library lookup tasks as identity work.
  - For web-only progress, capture the original 1880 Johnson Township, Clark County, Illinois census image for Nollie Frank / Frank in Marion McDonald Partlow's household, then capture original 1850 and 1860 Johnson Township census images for John H. Partlow.
