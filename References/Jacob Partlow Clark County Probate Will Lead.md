---
title: Jacob Partlow Clark County Probate Will Lead
aliases:
  - Jacob Partlow will
  - Jacob Newton Partlow probate
  - Jacob Partlow Clark County will
  - Jacob Partlow original will
date: 2026-06-22
tags:
  - "#copley-family"
  - "#partlow-line"
  - "#references"
  - "#probate"
  - "#research-in-progress"
---

# Jacob Partlow Clark County Probate Will Lead

This note preserves the June 2026 progression from derivative snippets to the **original will-book entry** for **Jacob Partlow**, Clark County, Illinois. The will-book record is now found and transcribed. The remaining live target is any separate probate packet: executor bond, letters testamentary, inventory, sale bill, settlement, distribution, docket entries, or loose estate papers.

Earlier raw snippet report: [Jacob Partlow probate/will lead output](/static/sources/claude-partlow/jacob-partlow-clark-county-probate-will-lead-claude-output.txt)

Original will report: [Jacob Partlow original will Ancestry output](/static/sources/claude-partlow/jacob-partlow-clark-county-original-will-ancestry-claude-output.txt)

Probate index / packet search report: [Jacob Partlow probate index and packet search output](/static/sources/claude-partlow/jacob-partlow-probate-index-packet-search-claude-output.txt)

Probate journal / newspaper follow-up report: [Jacob Partlow CC 166 and newspaper follow-up output](/static/sources/claude-partlow/jacob-partlow-cc166-newspaper-followup-claude-output.txt)

Companion reel follow-up report: [Jacob Partlow DGS 7655347 and 7655349 follow-up output](/static/sources/claude-partlow/jacob-partlow-dgs7655347-7655349-followup-claude-output.txt)

Guardian Book B final browse report: [Jacob Partlow Guardian Book B final browse output](/static/sources/claude-partlow/jacob-partlow-guardian-book-b-final-browse-claude-output.txt)

Offline request package: [Jacob Partlow offline probate request package](/static/sources/claude-partlow/jacob-partlow-offline-probate-request-package-claude-output.txt)

## Citation

Jacob Partlow will, 3 Sep 1867; Clark County, Illinois, Will Record, Book 1 (1859-1882), pp. approximately 158-159, image 280 of 1338; *Illinois, U.S., Wills and Probate Records, 1772-1999*, Ancestry.com, collection 9048, record ID `1196850`, image `007655352_00280`, https://www.ancestry.com/imageviewer/collections/9048/images/007655352_00280. Original microfilm: FamilySearch DGS `7655352` / Film `1308123`, "Probate, will, guardian, conservator and chancery records, 1840-1976," Illinois. County Court (Clark County), FamilySearch catalog `koha:317736`; microreproduction of original records at the Clark County Courthouse, Marshall, Illinois.

## Will-Book Details

- **Testator:** Jacob Partlow.
- **Residence:** Clark County, Illinois.
- **Will date:** 3 Sep 1867.
- **Probated / sworn in open court:** 26 May 1868.
- **Will book:** Clark County Will Record, Book 1, 1859-1882.
- **Image:** Ancestry image 280 of 1338.
- **Witnesses:** Jacob Geffs and Roger S. Davis.
- **Executor:** William W. Willison.
- **Clerk:** A. B. Briscoe / Briscor[e], County Clerk.
- **Signature:** Jacob signed by mark, rendered in the transcription as `Jacot X Partlow` / Jacob Partlow.

## Close Abstract

Jacob Partlow described himself as old but of sound mind and memory. He directed payment of funeral expenses and debts. He then gave **Polly my wife** all remaining estate for her life. After Polly's death and payment of her funeral expenses and debts, the will states that Jacob had given all his lawful heirs **fifty dollars apiece**. The residue of both personal and real estate was to go to **William**, except **Polly Ritenhouse / Ridenhouse** was to receive **a cow and bed**. Jacob appointed **William W. Willison** executor.

The probate attestation says witnesses Jacob Geffs and Roger S. Davis subscribed as attesting witnesses at Jacob's request and in his presence on 3 Sep 1867. They swore in open court on 26 May 1868 that Jacob was of full age, sound mind and memory, and under no constraint.

## Probate Index / Packet Search

A follow-up search of Ancestry collection 9048 found a probate index page for Jacob Partlow but did **not** locate a digitized packet, bond, inventory, sale bill, settlement, or distribution paper.

Index citation:

- **Repository:** Ancestry.com, *Illinois, U.S., Wills and Probate Records, 1772-1999*, collection 9048.
- **Record ID:** `1196688`.
- **Image:** `007655352_00192`, image 192 of 1338.
- **URL:** https://www.ancestry.com/imageviewer/collections/9048/images/007655352_00192?pid=1196688
- **Source section:** Clark County, Illinois, `INDEX TO ESTATE PAPERS`, labeled on film / in metadata as Probate Index Book A.

The relevant index entries on image 192 include:

- `PARTLOW James` - Will Record page 43, Probate Court B, page 36.
- `PARTLOW Jacob` - `(CC) 166`.
- `PARTLOW Jacob` - `188`.
- `PARTLOW John M.` - `1/28`.

Initial interpretation:

- The `188` entry points to the already-found Will Record Book 1 entry for Jacob's will.
- The `(CC) 166` entry is a County Court / probate-order lead, now resolved as **Probate Journal volume CC, page 166**; see below.
- The `PARTLOW John M. - 1/28` entry is separate and should not be assumed to be John H. without checking the underlying record.
- The searched Ancestry film section contains probate index and will-record images; no loose estate packet, executor bond, letters testamentary, inventory, sale bill, final settlement, receipt, distribution, probate docket, or guardianship paper for Jacob Partlow was found online in the checked images.

The report also identifies several FAN / collateral results:

- **Roger S. Davis** is likely Roger Spencer Davis, a young Civil War veteran and Johnson Township / Oak Point community member, not known kin.
- **William W. Willison** was a Bell Air / Bellair-area farmer and neighbor, a plausible trusted executor rather than proven kin.
- **Jacob Geffs** appears to be a Bellair / Johnson Township neighbor from a Virginia-origin family.
- **Polly Ritenhouse / Ridenhouse** appears in derivative reports as an approximately 10-year-old in Jacob's 1860 household. A Genealogy.com snippet points to a Rush County, Indiana marriage of Lavina Partlow to Wilson Reddinghause / Ridenhouse, making Polly plausibly a dependent granddaughter or grandniece, but this is not proven.

## Probate Journal / `CC 166` Follow-Up

A later follow-up checked the 1938 *Inventory of the County Archives of Illinois, No. 12 - Clark County (Marshall)*, available through Internet Archive at `inventoryofcount121938illi`.

The controlling inventory entry is **Entry 127, Probate Journal, 1820-**, pp. 64-65. The series is described as 43 volumes, with titles varying as **Probate Order Book** and **Probate Record**. The volume sequence is summarized as `2-5, 1820-56; 2 not labeled, B, 1849-60; A-Z, 1-9, 1860-; Zl, 1929-`.

Interpretation: the probate-index notation **`(CC) 166`** is a citation to **Clark County Probate Journal / Probate Order Book volume CC, page 166**. It is not a will-record page and not a box or loose-packet number. It most likely points to a County Court probate order, such as admission of the will, executor appointment, or later estate proceeding.

The same follow-up checked FamilySearch catalog `koha:317736` and Ancestry collection 9048. The seven reels include probate indexes, will records, guardian records, conservator records, administrator records, and some chancery books, but **not** the Probate Journal / Probate Order Book volume CC. The most relevant online companion reels left to check are:

| Film | DGS | Relevant content |
|---|---|---|
| 1308126 | `7655349` | Conservators Record Books 4-5, 1934-1964; Guardians Record Books B-E, 1865-1921 |
| 1308128 | `7655347` | Administrator's Record Books M-N, 1935-1964; Administrator-Guardian Index 1876-1893; Guardian Fee Books 1-3, 1900-1976 |

Conclusion: **Probate Journal volume CC is not included in the online FamilySearch / Ancestry seven-reel set.** The underlying `CC 166` entry requires Clark County courthouse or IRAD-EIU access unless a separate, newly identified digitization exists.

The same follow-up found that the **Index to Estate Papers** section of DGS `7655352` also lists estate boxes:

| Name | Box |
|---|---|
| Partlow, Isaac | 45 |
| Partlow, John B. | 45 |
| Partlow, Rachel | 46 |
| Partlow, Jacob | 46 |

This adds a physical packet pointer: **Jacob Partlow's estate file is box 46**. The packet itself was not found online.

### Newspaper Follow-Up

The Marshall Public Library Digital Archive / Advantage Archives was searched for 1868-1872 Clark County / Marshall papers. Search terms included Jacob Partlow, William W. Willison, Partlow estate, Polly Ritenhouse, Polly Ridenhouse, Jacob Geffs, and Roger S. Davis.

Findings:

- `Partlow` returned three 1868-1872 hits: two June 1869 William D. Crouch estate sale notices mentioning Nelson Partlow as grantee of a mill site, and an 1871 delinquent tax-list entry for Benjamin Partlow. None were Jacob Partlow estate notices.
- `Willison` returned one irrelevant 1872 item about a Miss Willison.
- Ritenhouse / Ridenhouse, Geffs, Roger S. Davis, and `Partlow estate` returned no relevant estate notices.
- Library of Congress newspaper search returned no 1868-1872 newspaper hits for Partlow.

Conclusion: no digitized 1868-1872 legal notice for Jacob Partlow's estate was found in the searched web-accessible newspaper paths. Newspaper notice work now requires a new repository, paid/newspaper access not already checked, or physical microfilm.

## Companion Reel Follow-Up: DGS `7655347` and `7655349`

A later follow-up checked the two remaining online companion reels most relevant to the `John M. - 1/28` and Ridenhouse guardianship questions.

### DGS `7655347`

DGS `7655347` contains Administrator's Record Books M-N, the Administrator-Guardian Index 1876-1893, Guardian Fee Books, and an executor / administrator bond section.

Findings:

- The Administrator-Guardian Index **P section** was checked at image `007655347_00409`; no Partlow entry appears.
- Surrounding / nearby index sections were checked at images `00405`, `00407`, `00408`, `00411`, `00425`, and `00430`; no Partlow entry was found.
- The index includes **Pettinger, John M.**, but no Partlow of any initial in the checked index sections.
- Page 28 of the executor / administrator bond volume, image `007655347_00650`, is an Emanuel Branner bond dated Jan. 1861, not a Partlow record.
- The same bond section has a collateral Partlow item at image `007655347_00640`, page 8: **James Partlow** appears as executor in a December 1860s bond, apparently for an Elizabeth Partlow estate. This is a useful collateral Partlow lead but not Jacob, John M., or John H.

Conclusion: **`PARTLOW John M. - 1/28` was not resolved from DGS `7655347`.** The likely candidate sections checked on this reel did not contain the underlying record. Do not repeat the image `00409` P-section or the bond-book page 28 check unless working from a new citation or higher-resolution reread.

### DGS `7655349`

DGS `7655349` contains Guardians Record Books B-E, 1865-1921, and later conservator records.

Findings:

- Guardian's Record Book B begins at image `007655349_00387` with chronological entries; no alphabetical front index was found at the opening of Book B.
- Page 28 of Guardian's Record Book B, image `007655349_00401`, is the appointment of Eliza A. Sarcuello as guardian for Nancy E. and Charles W. Marcroft; no Partlow or Ridenhouse appears.
- Images `00387-00407`, corresponding roughly to Book B pages 1-41 / Oct. 1865-Dec. 1866, were sampled or read for guardian and ward names. No Partlow, Ridenhouse, Ritenhouse, Rittenhouse, or Reddinghause entry was found in that range.

This was the interim result from the companion-reel pass. The final browse below supersedes the earlier partial status: Guardian Record Book B images `00387-00479`, roughly pages 1-185, are now checked negative through about February 1871.

## Guardian Record Book B Final Browse

A final web-only pass continued DGS `7655349`, Guardian Record Book B, from image `007655349_00408` through image `007655349_00479`. Image `00480` was used only as a calibration endpoint and showed February 1871.

Combined coverage now checked:

- Prior pass: images `00387-00407`, roughly Book B pages 1-41 / Oct. 1865-Dec. 1866.
- Final pass: images `00408-00479`, roughly Book B pages 43-185 / Dec. 1866-Feb. 1871.
- Total checked: images `00387-00479`, 93 images, roughly Book B pages 1-185.

No alphabetical index, section divider, or table of contents was observed. The book is a continuous chronological sequence of guardian appointments.

Result: **no Partlow, Ritenhouse, Ridenhouse, Rittenhouse, Reddinghause, Polly Ridenhouse, Mary Ridenhouse, Polly Ritenhouse, or Mary Ritenhouse entry was found** in the checked range, whether as guardian, ward, or deceased ancestor.

Conclusion: the online Guardian Record Book B search is now complete through about February 1871 and negative. Do not repeat DGS `7655349` images `00387-00479` unless working from a new citation or higher-resolution reread.

## Key Findings

### John H. / John Halleck Partlow

The original will-book entry does **not** name John H. Partlow or John Halleck Partlow. The only individually named people in the will are:

- Polly, Jacob's wife.
- William, residuary beneficiary.
- Polly Ritenhouse / Ridenhouse, recipient of a cow and bed.
- William W. Willison, executor.
- Jacob Geffs and Roger S. Davis, witnesses.

John H. remains supported by the 1883 Perrin biography and compiled Harry C. Partlow / FamilySearch material, not by this will.

### Lawful Heirs

The will refers to **all my lawful heirs** collectively but does not name them. The important clause is that Jacob had already given all lawful heirs **fifty dollars apiece**. This corrects the earlier snippet report, which had read the amount as $40.

Interpretation: Jacob recognized a broader lawful-heir class, but the will-book entry does not identify that class by name. Any estate packet, settlement, receipt, or distribution paper is now the best chance to identify those heirs.

### Polly / Mary

The will confirms the wife as **Polly**. It does not use the name Mary. Polly remains probably Mary / Polly Abrams Partlow by correlation with the indexed 1815 Indiana marriage and the 1860 household, but the will itself says only **Polly my wife**.

### William

The will names **William** as residuary beneficiary but does not give his surname, middle name, or relationship. William Franklin Partlow remains the leading identification by correlation with Jacob's 1860 household and derivative memorial evidence, but the will itself does not say "William Franklin Partlow" or "my son William."

### Polly Ritenhouse / Ridenhouse

The will gives **Polly Ritenhouse / Ridenhouse** a cow and a bed. Her relationship to Jacob is not stated. She is separate from wife Polly because the wife receives a life estate and this later clause carves out a specific bequest from William's residue.

Next target: identify Polly Ritenhouse / Ridenhouse in 1860-1880 Clark County records and test whether she was kin, ward, servant, granddaughter, step-kin, or unrelated.

### Witnesses and Executor

The witnesses were **Jacob Geffs** and **Roger S. Davis**. The executor was **William W. Willison**. The will does not identify any of them as kin. Their value is now FAN-club context: search Clark County deeds, tax lists, probate records, and census neighborhoods for proximity to Jacob Partlow, William Partlow, or Polly Ritenhouse / Ridenhouse.

## Evidence Assessment

This is an official Clark County will-book copy of Jacob Partlow's will and probate attestation. It is stronger than the prior derivative snippet trail and confirms the basic will details. It does not prove John H. Partlow as Jacob's son, does not identify Jacob's parents or birthplace, and does not repair the contradicted **Jacob -> Benjamin Partlow** link.

The will materially improves the research position by showing that Jacob had a broader lawful-heir class and that those heirs had received prior payments. The missing evidence is not the will text itself; it is any accompanying probate packet or settlement papers that may name the recipients.

The probate-index and CC follow-ups improve the control map by confirming that Jacob appears in the Clark County estate-paper index, that `PARTLOW Jacob - 188` points to the found will, that `PARTLOW Jacob - (CC) 166` means Probate Journal volume CC page 166, and that Jacob's physical estate file is indexed to **box 46**. The Probate Journal and box 46 packet are not online in the checked Ancestry / FamilySearch reel set. The web-accessible Guardian Record Book B search through February 1871 is now negative for Partlow / Ridenhouse variants.

## Remaining Access Path

The will-book image is online through Ancestry, and FamilySearch filmed the will record as DGS `7655352` / Film `1308123`. The remaining physical packet / court-book search should target:

- Probate Journal / Probate Order Book volume **CC**, page **166**.
- Estate file / packet **box 46**.
- Executor's bond.
- Letters testamentary.
- Probate docket / case number.
- Inventory.
- Sale bill.
- Final settlement.
- Receipts or distribution papers.
- Any guardianship or conservatorship files connected to Polly Ritenhouse / Ridenhouse.

Request wording:

> I am seeking the original probate packet / estate case file for **Jacob Partlow**, deceased, Clark County, Illinois. His will was dated **3 September 1867** and proved in open court **26 May 1868**. The will-book entry appears in Clark County Will Record, Book 1 (1859-1882), approximately pp. 158-159, Ancestry image 280 of 1338, FamilySearch DGS `7655352` / Film `1308123`. The Clark County Index to Estate Papers lists **Partlow, Jacob, box 46**, and the probate index also cites **Probate Journal / Probate Order Book volume CC, page 166**. The executor was **William W. Willison**, and witnesses were **Jacob Geffs** and **Roger S. Davis**. Please search Probate Journal volume CC page 166, estate packet box 46, and any surviving executor's bond, letters testamentary, inventory, sale bill, final settlement, estate distribution, receipt, probate docket, or loose packet papers.

Primary repositories:

- Clark County courthouse / clerk or circuit clerk, Marshall, Illinois.
- IRAD-EIU, Booth Library, Eastern Illinois University, Charleston, Illinois; the June 2026 request package gives phone `217-581-6093` and general archives email `archives@eiu.edu`.
- Ancestry / FamilySearch catalog `koha:317736` for the remaining online-accessible companion reels, especially DGS `7655347` and DGS `7655349`.
- Marshall Public Library / NewspaperArchive / physical microfilm only if a new newspaper path is available; the searchable Marshall archive and LOC searches are checked negative for Jacob estate notices.

## Offline Request Package

The next practical action is a repository request, not additional broad web browsing. The package below should be used to request the **likely** court-order entry behind `PARTLOW Jacob - (CC) 166` and any surviving loose estate papers indexed to **box 46**.

Contact priority:

| Priority | Repository | Contact | Use |
|---|---|---|---|
| 1 | IRAD at Eastern Illinois University, Booth Library, Room 1700, 600 Lincoln Avenue, Charleston, IL 61920 | Phone `217-581-6093`; email `archives@eiu.edu`; reported hours Mon-Fri, 9 a.m.-12 p.m. and 1-4 p.m., except state holidays | Ask whether IRAD holds Clark County Probate Journal / Probate Order Book volume CC and County Court estate packet box 46. |
| 2 | Clark County Courthouse, 501 Archer Avenue, Marshall, IL 62441 | County Clerk / Recorder `217-826-8311`; Circuit Clerk `217-826-2811` | Use if IRAD does not hold the volume or loose papers, or if the records remain in the courthouse vault. |

### Email to IRAD-EIU

```text
To: archives@eiu.edu
Subject: Research Request - Clark County Probate Records, Estate of Jacob Partlow (ca. 1868-1872)

Dear IRAD-EIU Archivist,

I am a genealogical researcher and am writing to request access to or copies of specific Clark County probate records that I believe may be held in your depository.

Record 1 - Probate Journal / Probate Order Book, Vol. CC, Page 166

The Clark County probate index in Ancestry.com collection "Illinois, U.S., Wills and Probate Records, 1772-1999" includes the entry "PARTLOW Jacob - (CC) 166." A follow-up check of the 1938 Inventory of the County Archives of Illinois, No. 12 - Clark County (Marshall), identifies the relevant series as the Probate Journal / Probate Order Book / Probate Record. I am seeking Probate Journal / Probate Order Book volume CC, page 166, which is likely a court order or probate journal entry relating to Jacob Partlow's estate. Until the actual page is examined, the precise nature of the entry cannot be confirmed.

Record 2 - Estate Packet / Estate Papers, Box 46

The Clark County Index to Estate Papers also points Partlow, Jacob to box 46. I would be grateful if you could please search for any surviving estate packet or estate papers for Jacob Partlow that may be filed in box 46, under his name, or in any associated estate-file sequence.

Subject of interest:
- Decedent: Jacob Partlow
- County: Clark County, Illinois
- Will date: 3 September 1867
- Probate date: 26 May 1868
- Will Record: Book 1, page 188
- Probate Journal / Probate Order Book lead: vol. CC, page 166
- Estate packet / estate papers lead: box 46
- Executor: William W. Willison
- Wife / life tenant: Polly Partlow
- Residuary heir named in the will: William
- Specific legatee: Polly Ritenhouse / Ridenhouse, cow and bed
- Will witnesses: Jacob Geffs and Roger S. Davis

If any loose papers survive, I am especially interested in the executor's bond, letters testamentary, inventory, appraisement, sale bill, creditor receipts, annual settlements, final account, or distribution receipts.

I am happy to pay any applicable reproduction or research fees. Scans by email are preferred if available, but paper copies by mail are also acceptable. If these records are not held by IRAD-EIU, I would appreciate any referral to the correct Clark County office or repository.

Thank you very much for your assistance.

Respectfully,
[Your Name]
[Your Address]
[Your Phone / Email]
```

### Fallback Request to Clark County

Use this only if IRAD-EIU does not hold the requested volume or packet.

```text
Clark County Circuit Clerk
Clark County Courthouse
501 Archer Avenue
Marshall, IL 62441
Phone: 217-826-2811

Subject: Records Request - Estate of Jacob Partlow, Clark County Probate, ca. 1868-1872

Dear Circuit Clerk,

I am writing to request access to Clark County probate records relating to the estate of Jacob Partlow, with proceedings believed to date to approximately 1868-1872.

Specifically, I am seeking:

1. Probate Order Book / Probate Journal, vol. CC, page 166. This volume-and-page citation appears in the Clark County probate index as "PARTLOW Jacob - (CC) 166." The entry is likely a court order or journal entry pertaining to the Partlow estate, though the precise content cannot be confirmed until the page is examined.

2. Estate packet / estate papers. I would be grateful if your office could please search for any surviving estate packet or estate papers for Jacob Partlow, which may be filed under his name, in a numbered box, possibly box 46, or in loose probate file folders from that era.

Identifying details:
- Decedent: Jacob Partlow
- Will dated: 3 September 1867
- Probated: 26 May 1868
- Will Record: Book 1, page 188
- Executor: William W. Willison
- Wife / life tenant: Polly Partlow
- Residuary heir named in the will: William
- Specific legatee: Polly Ritenhouse / Ridenhouse, cow and bed
- Witnesses: Jacob Geffs and Roger S. Davis

I am prepared to pay any applicable search or copy fees. If these records have been transferred to the Illinois Regional Archives Depository at Eastern Illinois University, please advise me so I may redirect my request accordingly.

Thank you for your time and assistance.

Respectfully,
[Your Name]
[Your Address]
[Your Phone / Email]
```

### Phone Script

```text
Hello, my name is [Name], and I am a genealogical researcher. I am trying to locate older Clark County probate records for the estate of Jacob Partlow, with proceedings dating to about 1868 to 1872.

I am looking for two items. First, Probate Order Book or Probate Journal volume CC, page 166. The Clark County probate index cites "PARTLOW Jacob - (CC) 166," and I believe it is likely a court order or journal entry for the Partlow estate, though I need the page checked to confirm the content.

Second, I would like to know whether any surviving estate packet or estate papers exist for Jacob Partlow. The estate-paper index points him to box 46, but I understand loose papers do not always survive.

Do you hold Clark County probate or county court records for that period? If so, what is the best way to request copies and pay any search or copy fees? If not, do you know whether these records remain at the Clark County courthouse or were transferred to IRAD-EIU?
```

Do not state that the packet survives until a repository confirms it. Use **"please search for any surviving estate packet / estate papers"** in all outreach.

## Next Research Targets

1. **Offline / physical:** Request Probate Journal / Probate Order Book **volume CC, page 166** and estate packet **box 46** from Clark County or IRAD-EIU.
2. **Completed online negative:** Guardian Record Book B on DGS `7655349`, images `00387-00479`, is checked negative for Partlow / Ridenhouse variants through about February 1871.
3. **Optional lower-priority web work:** Continue into later Guardian Record Books C-E only if pursuing a broad Ridenhouse / Partlow guardianship sweep beyond the likely window.
4. **Unresolved but bounded:** `PARTLOW John M. - 1/28` remains unresolved after checking the Administrator-Guardian Index P section and candidate page 28 bond / guardian-book records. Revisit only with a new candidate series or higher-resolution index reread.
5. **Collateral lead:** Abstract the James Partlow executor bond at DGS `7655347`, image `007655347_00640`, if broader Clark County Partlow probate context becomes useful.
6. Continue the separate original-record search for the reported **Mary Halleck** first marriage and **Isaac Partlow** brother relationship.
7. Retrieve the original Indiana marriage register entry for Jacob Partlow and Polly Abrams, 13 Sep 1815, FHL film `000549348`.
