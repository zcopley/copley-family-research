---
title: Anne Copley
date: 2026-04-13
tags:
  - "#copley-family"
  - "#generation-G24"
---

# Anne Copley (b. 1850; d. 1923)

## Biographical Profile
[[Anne Copley]] is a daughter of [[Michael Copley]] and [[Ann Copley]], reported born in **1850** (appears as 6 months old in 1850 household context). Family narrative states she **never married** and died in **1923**.

She should be distinguished from later relatives with similar names, including [[Anne Copley]] in John’s daughter generation (disambiguation needed in later cleanup phase).

## Family Relationships
- Parents: [[Michael Copley]], [[Ann Copley]]
- Spouse: none reported
- Children: none reported
- Siblings:
  - [[Mary Copley Quinn]]
  - [[John Copley]]
  - [[Catherine Kitty Copley Hannon|Catherine "Kitty" Copley Hannon]]
  - [[Bridget Bitty Copley Gillooly|Bridget "Bitty" Copley Gillooly]]
  - [[Margaret Copley]]
  - [[Thomas Tom Copley|Thomas "Tom" Copley]]
  - [[Sarah Copley]]

## Research Gaps
1. Exact birth and death certificate references.
2. Residence chronology after 1870.
3. Burial location and obituary.
4. Name disambiguation with younger Anne (John’s daughter).

## Acquisition Strategy
- Build a census-only baseline (1850-1920) for unmarried women named Anne/Ann Copley in Lewis County.
- Retrieve church burial records and cemetery transcriptions.
- Introduce a disambiguation convention in vault metadata (e.g., “Anne Copley (b.1850)”).

## Source Citations
1. *COPLEY HISTORY PART 1 final 2.pdf* (birth cohort, unmarried status, death year).
2. `/home/ubuntu/copley_research_analysis.md` (household and chronology context).
